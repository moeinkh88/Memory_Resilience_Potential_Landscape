...
LamAttrL(i)=(DX(XLattr+1,i)-DX(XLattr,i))/((C(end)-C(1))/NX);
LamAttrR(i)=(DX(XRattr+1,i)-DX(XRattr,i))/((C(end)-C(1))/NX);
end
yline(0)
xlabel('States (X)')
axis tight
...
text(1.5,.25,num2str(LamAttrL(1)), 'FontSize',12,'color',colors(1,:))
text(1.5,.2,num2str(LamAttrL(2)), 'FontSize',12,'color',colors(2,:))
text(4,.25,num2str(LamAttrR(1)), 'FontSize',12,'color',colors(1,:))
text(4,.2,num2str(LamAttrR(2)), 'FontSize',12,'color',colors(2,:))
...
DDQ=-diff(DX)/((C(end)-C(1))/NX);

% figure

for i=1:length(al)

p224=plot(Xall(1:end-1,i),DDQ(:,i),'color',colors(i,:));
% , 'LineStyle', 'none','Marker','.');
hold on
...
for i=1:length(al)
 ...
 [TF,p] = islocalmin(Q(:,i));
   ...
   MeanExit(:,i)=2*pi./sqrt(abs(DDQ(:,i).*DDQ(TF1,i))).*exp(abs(Q(Xmax,i)-Q(1:end-1,i))./16);
   p324=plot(Xall(1:end-1,i),MeanExit(:,i),'color',colors(i,:));
