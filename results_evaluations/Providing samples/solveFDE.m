function [LRdp,Lp,Hp,LamAttr,Q,Xall]=solveFDE(par,al,r1,r2)


xt=0:.00001:10; % steps for polynomial function
U=-polyint(par);
U1=polyval(U,xt);
TF=islocalmin(U1);
TF1=islocalmax(U1);
...
%          [t,x] = FDE_PI12_PC(al(J),F,t0,T,X0,h); 
                  [t,x] = FDE_PI2_IM(al(J),F,JF,t0,1,X0,h); 
    X(J,:)=x;
...
Hp(J)=Q(TF1,J); % height of potantial landscape on threshold (peak)
Lp(J,1)=0;
Lp(J,2)=Q(TF,J);  % height of potantial landscape in attractions
