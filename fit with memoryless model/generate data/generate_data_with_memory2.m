% generate data for the case when memory increase the potential depth
clc
clear 
...
bb=20:10:68; % define some values within multistability region to generate data with initial conditions around the unstable points
...
for i = 1:nb
    ...
    idx    = round(linspace(1, NtFull, 100*T+1));   % T+1 evenly‑spaced indices
...
xlabel('t');  ylabel('x(t)');  legend show;  box on

%%
save('herbivore_dynamics_memory7.mat','sol')
%% 
...
% write to CSV
writetable(tblAll,'herbivore_dynamics_memory7.csv')

fprintf('CSV written: %s (%d rows)\n', ...
        'herbivore_dynamics_memory7.csv',height(tblAll));
