...
paramSets = [
    struct('r',0.800000,'K',6.00000,'A',0.200000,'label','baseline'), ...
    % struct('r',0.739859,'K',5.88714,'A',0.0688134,'label','new set')
    % struct('r',0.7188,'K',5.6334,'A',0.0591,'label','new set')
    struct('r',0.57261,'K',5.82416,'A',0.111232,'label','new set')
];

% -------- common c‑range ------------------------------------------------
c_min = 0;         % left edge for parameter B
c_max = 5;         % right edge (make sure it’s large enough)
...
for k = 1:numel(paramSets)
    ...
    plot(c1,Eq1,'-','Color',col(3-k,:),'LineWidth',2);           % zero branch
    plot(c2,Eq2,'--','Color',col(3-k,:),'LineWidth',2,'HandleVisibility','off');           % upper (stable)
    plot(c3,Eq3,'-','Color',col(3-k,:),'LineWidth',2,'HandleVisibility','off');          % lower (unstable)
