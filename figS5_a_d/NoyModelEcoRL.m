...
if Rx(end)>xmax 
    P=P+0.0005;
    RX(i,:)=Rx;
% RS=(1-2*(abs(RX0-min(Rx)))/(abs(RX0-min(Rx))+RX0));
else
% a1=A*r-B;
% a2=r*(1-A/K);
% a3=-r/K;
% par=[a3,a2,a1];
% [Xall,Q,dp]=PotenDepth(par,al(i));
    EcoRL(i)=(P-0.0005);    
%         RX(i,:)=Rx;   
% DP(i)=dp(2); % dp(2) is for the right valley
    break
end
toc
end
