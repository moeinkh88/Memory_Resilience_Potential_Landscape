%% fit_hill_integer.m
% Parameter estimation for the integer‑order Hill‑type herbivore model
%   dx/dt = r*x*(1 - x/K) - b*x^q / (x^q + A^q)
%
...
%% ---------------- USER OPTIONS ------------------------------------------
fixed_rKA = false;      % true  → fit only the b‑values      (and keep q fixed)
                        % false → fit [r K A q] + all b‑values

% initial guesses (reasonable defaults for your data)
r0 = 0.8;  K0 = 6;  A0 = 0.1;
q0 = 1.5;                          % <‑‑ NEW: Hill exponent
...
%   p = [r K A q  b1 … b_nruns]  (when fixed_rKA is false)
...
if fixed_rKA
    ...
    p0 = [r0 K0 A0 q0 repmat(b0,1,nruns)];
    lb = [0  0  0  .2   zeros(1,nruns)]; % q ≥ 1 ensures sigmoid shape
...
if fixed_rKA
    rEst = r0; KEst = K0; AEst = A0; qEst = q0;
    bEst = pEst(:);
else
    rEst = pEst(1); KEst = pEst(2); AEst = pEst(3); qEst = pEst(4);
    bEst = pEst(5:end).';
end
...
save('fit_hill_results.mat','pEst','rEst','KEst','AEst','qEst','bEst', ...
     'resnorm','exitflag');
fprintf('\nAll results saved in fit_hill_results.mat\n');
...
% Stack residuals from every trajectory into a single column vector
    if fixed_rKA
        r = 0.8; K = 6; A = 0.1; q = 1.5;    % <-- EDIT if your fixed values differ
        bAll = p(:);
    else
        r    = p(1);  K = p(2);  A = p(3);  q = p(4);
        bAll = p(5:end);
    end
...
for ir = 1:nruns
    ...
    ode = @(t,x) rEst*x.*(1 - x./KEst) - ...
                 b*x.^qEst./(x.^qEst + AEst.^qEst);
...
title('Hill‑type herbivore model – fitted integer‑order trajectories vs. data');
