...
U=-polyint(par(1:3)); % potential of polynomial fun; -int(poly)
U1=polyval(U,xt);
TF=islocalmin(U1);
TF1=islocalmax(U1);
...
t0=0;
h=0.005;
T=50; % final time for the dynamics
...
% solve from 4 initial states to cover all states for potential landscape
                  ...
                  [MLt,MLX] = FDE_PI2_IM(al(j),F,JF,t0,T,MLX0,h/5,par,1e-10,200); 
    
                  [Mt,MX] = FDE_PI2_IM(al(j),F,JF,t0,T,MX0,h,par); 
    
                  [Rt,RX] = FDE_PI2_IM(al(j),F,JF,t0,T,RX0,h/5,par); 
