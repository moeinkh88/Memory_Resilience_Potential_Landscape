...
dataFile = 'herbivore_dynamics_memory8.csv';
...
r0 = .57261;  K0 = 5.82416;  A0 = 0.111232;
b0 = 0.5;                 % initial guess for every b_i

% solver / optimiser tolerances
% ---------- solver / optimiser tolerances (HIGH-PRECISION) --------------
odeOpts = odeset('RelTol',1e-12, ...
                 'AbsTol',1e-14, ...
                 'MaxStep',0.1, ...          % limit step growth
                 'NonNegative',1);           % biological state ≥0
setappdata(0,'odeOpts',odeOpts);             % share globally
...
if fixed_rKA
    ...
    lb = [0  0  0   zeros(1,nruns)];
    ub = [1 6  1  8*ones(1,nruns)];
end
...
bestCost = inf;   bestP = p0;
for restart = 1:5
    if restart>1, p0 = lb + (ub-lb).*rand(size(p0)); end
    pTry = lsqnonlin(obj,p0,lb,ub,lsqOpts);
    cost = norm(obj(pTry))^2;
    if cost<bestCost, bestCost=cost; bestP=pTry; end
end
pEst = bestP;
...
save('fit_integer_results_monostable_included.mat','pEst','rEst','KEst','AEst','bEst', ...
     'resnorm','exitflag');
fprintf('\nAll results saved in fit_integer_results_monostable_included.mat\n');
...
% Stack residuals from every trajectory into a single column vector
    if fixed_rKA
        r = 0.8; K = 6; A = 0.2;      % <-- EDIT here if your fixed values differ
        bAll = p(:);
    else
        r    = p(1);  K = p(2);  A = p(3);
        bAll = p(4:end);
    ...
    for ir = 1:numel(runIDs)
        ...
        for ic = [1 2]                         % each initial condition
            ...
            sol  = ode15s(ode,[t(1) t(end)],x0,getappdata(0,'odeOpts'));   % in the plot block
            xSim = deval(sol,t);
...
function x = ode_atTimes(odefun,tspan,x0)
    sol = ode15s(odefun,[tspan(1) tspan(end)],x0,getappdata(0,'odeOpts'));
    x   = deval(sol,tspan).';
end
