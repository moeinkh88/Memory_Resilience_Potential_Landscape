...
EngRL(i)=(2*(abs(Xmin-min(x)))/(abs(Xmin-min(x))+Eps)-1)/(t(ind(1))); % computing resilience
end
...
axis([0 T min(min(RX(:,:))) max(max(RX(:,:)))])
...
% [Q1LM,Xall1LM]=solveFDEM1(par,al(2)); % Potential with distribution left side
[Q1LM,Xall1LM,Q1RM,Xall1RM]=solveFDEM(par,al(2)); % Potential with distribution right side
...
if r1>0 && r2>0 && isreal(r1) && isreal(r2)
    [Xall1,Q1]=PotenDepth(par,al(1)); % Potential without distribution
else
    [Q1,Xall1]=solveFDE(par,al(1)); % Potential with distribution
end
...
[Xall,Q]=PotenDepth(par,al(1)); % Potential without distribution
[XallM,QM]=PotenDepth(par,al(2)); % Potential without distribution
...
ind=find(abs(Xall1-Xall1LM(end))<.001);ind=ind(1);
