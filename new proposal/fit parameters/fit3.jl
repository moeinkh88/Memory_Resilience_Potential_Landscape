...
@model function fit_herbivory(obs, times, problems)
    ...
    r ~ LogNormal(log(0.8), 0.1)        # intrinsic growth rate (positive)
    K ~ LogNormal(log(6.0), 0.2)       # carrying capacity (positive)
    A ~ LogNormal(log(0.1), 0.7)       # half-saturation constant for grazing (positive)
    # r  ~ truncated(Normal(0.8, .55), 0, Inf)
    # K  ~ truncated(Normal(6.0, 20.0), 0, Inf)
    # A  ~ truncated(Normal(0.1, 5.0),  0, Inf)
    # Hierarchical: independent prior for each group's grazing rate b[i]
    b ~ filldist(LogNormal(log(1.0), 0.3), size(obs, 1))   # vector of grazing rates for each group (all > 0)
    # Shared observation noise (std dev)
    σ ~ Truncated(Cauchy(0, 1), 0.0, Inf)       # noise scale (std) with a relatively uninformative prior
